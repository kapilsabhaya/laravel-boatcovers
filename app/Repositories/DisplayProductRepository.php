...
class DisplayProductRepository 
{
    public function getProduct(array $data,$make,$model,$year) {
        ...
        $products = Product::with(['getCategory','media' => function($query){ $query->orderBy('sort_order')->limit(1); },'productOption.optionValue' => function($que) { $que->orderBy('option_value.sort_order'); }])->where('category_id',$category->category_id)->where('product.is_active',true)->orderBy('sort_order')->get();
         return ['products' => $products , 'make' => $make['name'] ,'model' => $model['model_name'], 'year' => $year];
    }
